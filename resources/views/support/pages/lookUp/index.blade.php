        ...
        <div class="kt-container ">
            <div class="kt-subheader__main lookUpSubheader">
                ...
                <div class="kt-subheader__wrapper">
                    <a class="btn btn-success btn-elevate btn-icon-sm add_new_lookup_fields" data-id="{{$sections[0]->id}}" id="addNewLookup"><i class="la la-plus"></i>
                        Lookup
                    </a>
                </div>
                        ...
                        <div class="kt-section__content kt-section__content--border kt-section__content--fit sideSectionContent sideValidNav" style="margin-top:20px!important;">
                            <ul class="kt-nav kt-nav--bold kt-nav--md-space kt-nav--v3" role="tablist" id="lookupSectionsContainer">
                                ...
                                <li class="kt-nav__item  validation_sections_element "  role="tab" @if($key === 0) style="background: rgba(93, 88, 209, 0.16);"@endif>
                                    <a class="kt-nav__link load_lookup @if($key === 0)lookup_active @endif" data-id="{{$section->id}}" data-toggle="tab" data-route="/lookupTable/{{$section->id}}"  style="padding-top:0; padding-left:0; width: 285px;" >
                                        <span class="kt-nav__link-text">{{ucfirst($section->section)}}</span>
                                    </a>
                                    <div class="section_edit_delete">
                                        <i class="la la-edit validation_section_li_edit " data-route="/lookup/editSectionModal/{{$section->id}}" data-toggle="modal" data-target="#sy_global_modal"style="font-size: 16px; padding: 0px 9px; @if($key === 0) display:block;@endif"></i>
                                        <i class="la la-trash validation_section_li_delete delLookupSection" data-route="/lookup/deleteSection/{{$section->id}}" style="font-size: 16px;@if($key === 0) display:block;@endif"></i>
                                    </div>
                                </li>
                                @endforeach
        ...
        // datasource definition
            ...
            columns: [
                ...
                template: function(row, index, datatable) {
                    ...
                    return     '<a href="javascript:void(0);" class="btn btn-hover-brand btn-icon btn-pill model_load add_lookup_group" data-id="'+row.id+'" data-route="/lookup/addGroup/'+row.code+'" data-toggle="modal" data-target="#sy_global_modal"  title="Add lookup group">\
        ...
        // datasource definition
            ...
            columns: [
                ...
                template: function(row, index, datatable) {
                    ...
                    var dropup = (datatable.getPageSize() - index) <= 4 ? 'dropup' : '';
                    return     '<a href="javascript:void(0);" class="btn btn-hover-brand btn-icon btn-pill model_load edit_lookup" data-id="'+row.id+'" data-route="/lookup/edit/'+row.id+'" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
...
$(document).off('click', '.delLookupButton').on('click', '.delLookupButton', function(e){
        ...
        delConfirm({
            url:"/lookup/deleteLookup/"+lookup_id,
            header: $(`#${table}`),
        });
