...
<script type="text/javascript">
    ...
    $('#t_userpermissionstable').KTDatatable({
        ...
        data: {
            ...
            source: {
                read: {
                    url: '/getDatatable/'+'userPermissions',
                    method: 'GET',
                    params: {
                        "_token": "{{ csrf_token() }}"
                    },
        ...
        columns: [
            ...
            }, 
            {
                ...
                template: function(row, index, datatable) {
                    ...
                    return '<a  id="edit_userPermission" class="btn btn-hover-brand btn-icon btn-pill model_load" data-route="rolePermissions/edit/userPermission/'+row.id+'" href="JavaScript:void(0);" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
    ...
    $(document).off('click', '#edit_userPermission').on('click', '#edit_userPermission', function(e){
		...
		let id= $(this).data('id');
		delConfirm({
			url:"rolePermissions/delete/userPermission/"+id,
