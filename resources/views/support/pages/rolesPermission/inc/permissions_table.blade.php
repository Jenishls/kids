...
$('#t_permissionstable').KTDatatable({
  // datasource definition
	  ...
	  columns: [
        ...
        return     '<a  id="edit_permission" class="btn btn-hover-brand btn-icon btn-pill model_load" data-route="rolePermissions/edit/permission/'+row.id+'" href="JavaScript:void(0);" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
...
$(document).off('click', '#edit_permission').on('click', '#edit_permission', function(e){
		...
		let id= $(this).data('id');
		delConfirm({
			url:"rolePermissions/delete/permission/"+id,
