	...
	// datasource definition
			...
			},
		{
			...
			template: function(row, index, datatable) {
				...
				return     '<a id="edit_page" class="btn btn-hover-brand btn-icon btn-pill model_load" data-route="rolePermissions/edit/page/'+row.id+'" href="JavaScript:void(0);" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
	...
	$(document).off('click', '#add_page').on('click', '#add_page', function(e){
		...
		let id= $(this).data('id');
		delConfirm({
			url:"rolePermissions/delete/page/"+id,
			header: $('#t_pagestable')
		});
	});
