...
$('#t_rolepermissionstable').KTDatatable({
  // datasource definition
	  data: {
	    ...
	    source: {
	      read: {
	        url: '/getDatatable/'+'rolePermissions',
	        method: 'GET',
	        params: {
				"_token": "{{ csrf_token() }}"
				},
	  ...
	  columns: [
        ...
        return     '<a  id="edit_rolePermission" class="btn btn-hover-brand btn-icon btn-pill model_load" data-route="rolePermissions/edit/rolePermission/'+row.id+'" href="JavaScript:void(0);" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
...
$(document).off('click', '#edit_rolePermission').on('click', '#edit_rolePermission', function(e){
	...
	let id= $(this).data('id');
	delConfirm({
		url:"rolePermissions/delete/rolePermission/"+id,
