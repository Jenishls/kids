<div id="bankBoxTable" data-id="{{$id}}">
                ...
                <div class="rp_btn col-xl-4 order-1 order-xl-1" style="display:inline-flex;">
                     <div class="btn-toolbar col-6" role="toolbar" aria-label="..." style="justify-content:flex-end !important;">
                        <div class="btn-group mr-2" role="group" aria-label="...">
                            <button type="button" class="btn btn-pill btn-warning pageload" data-route="/admin/bankmaster"><i class="la la-arrow-left"></i>Back</button>
                        </div>
    ...
    <script>
        var bankboxid = $('#bankBoxTable').attr('data-id');
        var bankBoxTable = $('#bankBoxTable').KTDatatable({
            // datasource definition
            data: {
                type: 'remote',
                source: {
                    read: {
                        url: `/admin/bankmaster/bankbox/list/${bankboxid}`,
                        method: 'get',
                        map: function(raw) {
                            // sample data mapping
                            var dataSet = raw;
        ...
        //     alert($(this).val());
        // })
        bankBoxTable.on('kt-datatable--on-check', function(event, args){
            console.log(event, args);
            bankBoxTable.setActive($this);
        });
        ...
        $(document).ready(function() {
            ...
            const basicSearch = (advanced = false, cb = "default cb") => {
                if (advanced) {
                    bankBoxTable.setDataSourceParam("query", {});
                } else {
                    let sanitized = bankBoxTable.getDataSourceQuery('query');

                    if (sanitized.advanced) delete sanitized.advanced;
                    bankBoxTable.setDataSourceParam("query", sanitized);
            ...
            $('#advSearchBtn').on('click', function(e) {
                ...
                basicSearch(true, () => {
                    bankBoxTable.search(reducedData, "advanced");
                });
            ...
            $('.basic--search').change(function(e) {
                ...
                basicSearch(false, () => {
                    bankBoxTable.search($(this).val(), $(this).attr('name'));
                });
            });
            $('#clientTableReload').on('click', function(e) {
                e.preventDefault();
                $('#bankBoxTable').KTDatatable().reload();
            });
