...
<div class="modal-dialog custom_dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" style="margin-left:2.5rem;" id="cWizardTitle">Update Tasks</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="color:cornsilk;">
            </button>
        </div>
        <div class="modal-body" style="padding:0px !important;" id="taskUpdateWizard">
            <div class="kt-portlet kt---tabs" style="margin-bottom:0px !important;">
                <div class="kt-portlet__head" style="padding:0px !important;">
                    <div class="kt-portlet__head-toolbar" style="width:100%;padding: 0 25px;">
                        <ul class="nav  nav-tabs-line" role="tablist"  style="width: 100%;display:flex;">
                            <li class="wizard-tabs active" data-validateurl="/admin/tasks/validate/tasks/add_task" data-target="#taskFormSection" style="flex:1;margin-right:0px !important;">
                                <a class="nav-link modal_tab_headers"  data-toggle="tab" href="javascript:" role="tab" aria-selected="false">
                                    <i class="la la-user mr-1"></i>Tasks
                                </a>
                            </li>
                            <li class="wizard-tabs col-sm-12"  data-validateurl="/admin/tasks/validate" data-target="#assignMemberSection" style="flex:1;margin-right:0px !important;">
                                <a class="nav-link modal_tab_headers"  data-toggle="tab"  href="javascript:" role="tab" aria-selected="false">
                                    <i class="flaticon-map mr-1"></i> Assign Members
                                </a>
                            </li>
                            <li class="wizard-tabs col-sm-12"  data-validateurl="/admin/tasks/validate"  data-target="#attachmentSection" style="flex:1;margin-right:0px !important;">
                ...
                <div class="kt-portlet__body" style="background:#e5f7ff !important; padding:0px !important;"> 
                    <form class="kt-form kt-form--label-right" id="taskUpdateForm">   
                        @csrf
                        <div class="tab-content">
                            <div class="wizard-panel active" id="taskFormSection" data-url="/admin/tasks/update/info" role="tabpanel" style="background:#e5f7ff !important;">
                                <div class="kt-portlet__body" style="padding:25px;padding-bottom:0px !important;">
                                    <input type="hidden" name="id" value="{{$task->id}}">
                                    @include('supportNew.pages.tasks.modal.include_edit.task_info')
                                </div>
                            </div>

                            <div class="wizard-panel" id="assignMemberSection" data-url="/admin/tasks/update/members" style="background:#e5f7ff !important;">
                                <div class="kt-portlet__body" style="padding: 20px 25px 0!important;">
                                    <input type="hidden" name="id" value="{{$task->id}}">
                                    @include('supportNew.pages.tasks.modal.include_edit.task_members')
                                </div>
                            </div>

                            <div class="wizard-panel" id="attachmentSection" data-url="/admin/tasks/update/attachments" style="background:#e5f7ff !important;">
                                    <input type="hidden" name="id" value="{{$task->id}}">
                                    @include('supportNew.pages.tasks.modal.include_edit.task_attachments')
                            </div>
                        </div>
                            ...
                            <div class="kt-form__actions">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 wizard--btn kt-align-right ml-auto" data-wizard-step="save">
                                        <button type="" class="btn btn-sm btn-pill btn-success" id="btnUpdateTask">Update <i class="la la-save"></i></button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    var taskUpdate = $('#taskUpdateWizard').WizardForm();
    
    taskUpdate.on('nextBtn', function(wizard){
        let abc = wizard.currentStep().attr("data-validateurl");
        if(wizard.currentStep().attr("data-validateurl") && abc != "muted"){
            wizard.sendAjax(wizard.currentStep().data("validateurl"));
        }
        let section = wizard.currentStep().data("target");
        console.log(section);
        console.log(section);
        supportAjax({
            url:$(section).attr("data-url"),
            method:"POST",
            data: $(section).find(':input').serializeArray(),
        },(response) =>{
    ...
    $('#btnUpdateTask').off('click').on('click', function(e){
    e.preventDefault();

        let data = $("#attachmentSection").find(':input').serializeArray();
        supportAjax({
            url:'/admin/tasks/update/attachments',
            method:'Post',
            data
        }, function(resp){
            $('#cModal1').modal('hide');
