<form id="purchaseOrderAddForm" data-provider-type="Service Provider">
    {{-- {{dd(auth()->user()->client)}} --}}
                        ...
                        <div class="card-title collapsed" data-toggle="collapse" data-target="#collapseOne4" aria-expanded="false" aria-controls="collapseOne4">
                            <div class="row" style="width:80%;">
                                ...
                                <div class="form-group col-xl-2 col-sm-6">
                                    ...
                                    <div class="input-group" >
                                        <input type="text" class="form-control" value="{{$po_number}}"  name="inv_no" disabled>
                                        <input type="text" class="form-control" value="{{$po_number}}"  name="po_number" hidden>
                                    </div>
                                </div>
                                <div class="form-group col-xl-2 col-sm-6">
                                    <label class="kt-font-bolder" style="color:#505054">Expiry Date</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" value="{{default_exp_date()}}" name="exp_date" id="expiryDate">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card mb-20" style="border:1px solid #fafafa;border-radius:4px !important;padding:20px;">
                    ...
                    <div class="row" style="padding:20px; padding-bottom:25px;">
                        ...
                        <div class="col-lg-6 kt-align-right">
                            ...
                            <button class="btn btn-pill btn-success" id="createPurchaseOrder"><i class="la la-save"></i>Save</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    // });
     ...
     $(document).off('click', '#createPurchaseOrder').on('click', '#createPurchaseOrder', function(e){
        e.preventDefault();
        removeExtraRows();
        // return;
        let add_purchase_order = $('#purchaseOrderAddForm');
        let data = $('#purchaseOrderAddForm').serializeArray();
        supportAjax({
   ...
   $(document).on('change','input[name="billed_amt[]"], input[name="approved_amt[]"], input[name="quoted_amt[]"]', function(e){
        ...
        let pid = qtycontainer.closest('td').siblings().children().find('input[name="product_id[]"]');
        // console.log($(pid).val() == null, $(pid).val() == '', $(pid).val() == 0);
        if ($(pid).val() == null || $(pid).val() == '' || $(pid).val() == 0) {
            return;
        }
