	...
	<script type="text/javascript">
			...
			columns: [{
...

				{
					...
					template: function(row, index, datatable) {
						...
						return '<a class="btn btn-hover-brand btn-icon btn-pill get_profile" onclick="event.preventDefault();" data-route="/admin/users/userProfile/' + row.id + '"><i class="la la-eye" title="View profile"></i></a>\<a href="javascript:void(0);" class="btn btn-hover-brand btn-icon btn-pill model_load edit_user" data-route="/admin/users/edit/' + row.id + '" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
								<i class="la la-edit"></i>\
							</a>\<a class="btn btn-hover-brand btn-icon btn-pill pageLoad changePassword" data-toggle="modal" data-route="/admin/users/changePassword/' + row.id + '" data-id="' + row.id + '" title="Update password"><i class="fas fa-key" style="font-size: 1.1rem;"></i></a>\
		...
		$(document).ready(function() {
			...
			$('.searchRedo').on('click', function(e){
				...
				usersTable.setDataSourceParam("query",{});
				$('#t_userstable').KTDatatable().reload();
				localStorage.removeItem("t_userstable-1-meta");
			});
		...
		$(document).off('click', '.exportTo').on('click', '.exportTo', function(e) {
			...
			window.open('export/users/' + $(this).attr('data-export-to'))
		});
