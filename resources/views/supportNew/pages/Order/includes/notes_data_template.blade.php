        ...
        <div class="kt-notes__content">
            <div class="row mb-3" style="border-bottom: 1px dashed #d8d5d5;">
                ...
                @if(auth()->user()->id == $note->userc_id)
                    ...
                    <a href="javascript:void(0);" class="btn btn-clean btn-sm btn-icon btn-icon-md deleteNote" data-route="/admin/order/note/delete/{{$note->id}}">
...
<script>
    $('.editNote').off('click').on('click', function(e){
        ...
        $('#orderNoteAddForm').addClass('editMode').attr('data-noteid', noteId);
    })
    $('.deleteNote').off('click').on('click', function(e){
        ...
        confirmAction({
                ...
                $('#order_note_templater').html(response);
