                ...
                <div class="col-md-12">
                  @if(auth()->user()->id == $note->userc_id)
                    <span class="pull-right" style="padding-left: 10px;">
                       ...
                       <a href="javascript:void(0);" class="btn btn-clean btn-sm btn-icon btn-icon-md deleteNote" data-route="/admin/client/deletenote/{{$note->id}}">
...
<script>
  $('.editNote').off('click').on('click', function(e){
    ...
    $('#clientNoteAddForm').addClass('editMode').attr('data-noteid', noteId);
  })
  $('.deleteNote').off('click').on('click', function(e){
    ...
    confirmAction({
            ...
            $.get(url, {
              ...
              $('#client_note_templater').html(response);
