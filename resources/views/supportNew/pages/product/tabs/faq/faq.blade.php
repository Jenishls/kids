        ...
        <div class="card">
            <div class="card-header" id="faqHeading3">
                <div class="card-title collapsed" data-toggle="collapse" data-target="#faq{{$loop->iteration}}" aria-expanded="false" aria-controls="faq{{$loop->iteration}}">
                    {{$faq->question}}
                    <div class="ml-auto">
                        @if (!$faq->faqReplies['answer'])
                        <button type="button" class="btn btn-hover-brand btn-elevate-hover btn-icon btn-sm btn-icon-md btn-circle reply_faq" data-url = "admin/products/tab/faq/reply/{{ $faq->id }}">
                        ...
                        @endif
                        <button type="button" class="btn btn-hover-brand btn-elevate-hover btn-icon btn-sm btn-icon-md btn-circle del_faq" data-url ="admin/products/tab/faq/delete/{{$faq->id}}">
            ...
            <div id="faq{{$loop->iteration}}" class="collapse" aria-labelledby="faqHeading3" data-parent="#faq">
                <div class="card-body" >
                    @if ($faq->faqReplies['answer'])
                    <div class="faqreply">
                        {{ $faq->faqReplies['answer']}}
                    </div>

                    <div id="faq_container" style="display: none;">
                        <form class="kt-form kt-form--label-right editReplyForm">   
                            @csrf
                            <textarea class="form-control" name="answer" rows="10">{{$faq->faqReplies['answer']}}</textarea>
                            <button type="" class="btn btn-default btn-sm btn-pill cancelReply" style="margin-top: 10px; margin-right: 10px;">Cancel</button>
                            <button type="" class="btn btn-success btn-sm btn-pill updateReply" data-url="/admin/products/tab/faq/reply/update/{{$faq->faqReplies['id']}}" style="margin-top: 10px;">Edit</button>
                        </form>
                    </div>
                    @else
                    No Replies made Yet.
    ...
    $(document).off('click','.updateReply').on('click','.updateReply',function(e){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $.ajax({
                        method: "get",
                        url: "admin/products/tab/faq/{{$product->id}}",
                        beforeSend: function () {
