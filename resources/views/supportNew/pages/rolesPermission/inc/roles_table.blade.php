<div class="row search_row">
	...
	<div class="rp_btn col-xl-4 order-1 order-xl-2">
		@canAccess('Roles & Permissions', 'add')
		<div class="kt-subheader__wrapper">
			<button type="button" class="btn btn-bold btn-label-brand btn-sm add_field" data-toggle="modal" data-target="#sys_icon_model" id='add_field'>Add Role</button>
		</div>
		@endcanAccess
	</div>
</div>
...
$('#t_rolestable').KTDatatable({
  // datasource definition
	  ...
	  columns: [
        ...
        return     '<a id="edit_role" class="btn btn-hover-brand btn-icon btn-pill model_load" data-route="/admin/rolePermissions/edit/role/'+row.id+'" href="JavaScript:void(0);" data-toggle="modal" data-target="#sy_global_modal"  title="Edit details">\
...
$(document).off('click', '#edit_role').on('click', '#edit_role', function(e){
		...
		let id= $(this).data('id');
		delConfirm({
			url:"/admin/rolePermissions/delete/role/"+id,
			header: $('#t_rolestable')
		});
	});
