...
class ImportController extends Controller
{
    ...
    {
        $headers = array_map('strtolower', $converter->header);
    ...
    public function importFile(Request $request, $fullpath)
    {
        ...
        // dd($request->skip_rows);
        // $importer = new CsvImporter($fullpath,true);
        // dd($importer);
        $converter = new CsvImporter($fullpath);
        $datas = $converter->convert()->getDatas();
        switch ($request->table) {
            case 'zip_codes':
                $mapperData = $this->mapperZipArray();
                $check = $this->checkFile($mapperData, $converter);
                if (sizeof($check) !== sizeof($mapperData)) :
                    $zip = $this->utf8_encodeArray($this->importMapperGlobal($datas, $mapperData));
                    $save = $this->insertZip($zip, $request->table);
                else :
    ...
    private function insertZip(array $zips, $table)
    {
        if ($table == 'zip_codes') :
    ...
    public function importMapper(array $array)
    {
        ...
        foreach ($array as $a) {
            if (!in_array($a['Establishment #'], $already)) :
                ...
                $formatter['personal_email'] = 'kkc' . random_string() . '@systha.com';
    ...
    private function mapperZipArray()
    {
        return [
            'city' => 'city',
            'zipcode' => 'zipcode',
